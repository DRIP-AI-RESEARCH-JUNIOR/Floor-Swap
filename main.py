...
my_parser.add_argument('--tile', '--t', action='store',
                       type=str, required=True)
my_parser.add_argument('--input', '--i', action='store',
                       type=str, required=True)
my_parser.add_argument('--output', '--o', action='store',
                       type=str, required=True)
my_parser.add_argument('--weight', '--w', action='store',
                       type=str, required=True)
...
print(torch.max(input), torch.min(input), torch.mean(input),
      torch.max(output), torch.min(output), torch.mean(output))
...
# Network Builders
encoder_path = args.weight+"/encoder_epoch_30.pth"
decoder_path = args.weight+"/decoder_epoch_30.pth"
...
def swapperTile(floor_idx, original, tile, mode="RGB"):
    print("Swapping started")
    original = original.copy()
    original[floor_idx, :] = tile[floor_idx, :]
    return original

# Load and normalize one image as a singleton tensor batch


def pred_evaluator(input):
    print("prediction under process")
    ...
    return floor_idx, img_original


def tiles_generator(path, pred):
